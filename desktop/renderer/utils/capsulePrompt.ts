/**
 * CAPSULE PROMPT BUILDER (Renderer Client-Side Utility)
...
export function generateContinuationPrompt(capsule: any): string {
  ...
  if (Array.isArray(taskState.important_decisions) && taskState.important_decisions.length > 0) {
    const validDecisions = taskState.important_decisions.filter((d: any) => typeof d === "string" && d.trim());
  ...
  if (Array.isArray(taskState.constraints) && taskState.constraints.length > 0) {
    const validConstraints = taskState.constraints.filter((c: any) => typeof c === "string" && c.trim());
  ...
  if (Array.isArray(taskState.pending_work) && taskState.pending_work.length > 0) {
    const validPending = taskState.pending_work.filter((p: any) => typeof p === "string" && p.trim());
  ...
  if (Array.isArray(taskState.relevant_files) && taskState.relevant_files.length > 0) {
    const validFiles = taskState.relevant_files.filter((f: any) => typeof f === "string" && f.trim());
